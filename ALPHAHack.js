//Made by Arceus_Matt (c)Copyright 2015.
...
function dip2px(dips){
 ...
 function checkVersion() {
    var r  = new java.lang.Runnable() {
        run: function() {
            try {
                var urls= new java.net.URL("//VERSION here from github");
                ...
                clientMessage("§[Alpha] Update check failed ");
                if(err=="JavaException: java.net.UnknownHostException: raw.githubusercontent.com") {
                                clientMessage("§9[Alpha] No internet connection.");
                            }
                            else {
                                clientMessage("§9Error: \n" + err);
                            } 
            }
        }
    }
        ...
        upd.setPositiveButton("Update", new android.content.DialogInterface.OnClickListener() {
            onClick: function(par1) {
                var ru  = new java.lang.Runnable() {
                    run: function() {
                        try {
                            var urls = new java.net.URL("//staff - code from github");
                            var check = urls.openConnection();
                            check.setRequestMethod("GET");
                            check.setDoOutput(true);
                            check.connect();
